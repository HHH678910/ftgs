...
def _resolve_frame_image_path(scene_root: str, frame_file_path: str, extension: str) -> str:
    """
    Resolve image path from transforms entry with robust fallbacks.

    Supports entries like:
    - images/0001/cam0001_000000
    - images/0001/cam0001_000000.png
    - 0001/cam0001_000000
    - /abs/path/to/image.png
    """
    normalized = frame_file_path.replace("\\", "/")
    if normalized.startswith("./"):
        normalized = normalized[2:]

    raw_path = Path(normalized)
    has_ext = raw_path.suffix != ""
    variants = [raw_path] if has_ext else [raw_path.with_suffix(extension)]

    candidates = []
    if raw_path.is_absolute():
        candidates.extend(variants)
    else:
        # Most common layout: relative path from source_path.
        candidates.extend([Path(scene_root) / v for v in variants])
        # Fallback: transforms sometimes omit "images/" prefix.
        for v in variants:
            if not (len(v.parts) > 0 and v.parts[0] == "images"):
                candidates.append(Path(scene_root) / "images" / v)

    for c in candidates:
        if c.exists():
            return str(c)

    # Keep deterministic fallback for clearer errors downstream.
...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png", time_duration=None, frame_ratio=1, dataloader=False):
    ...
    def frame_read_fn(idx_frame):
        ...
        image_path = _resolve_frame_image_path(path, frame["file_path"], extension)
        cam_name = os.path.relpath(image_path, path)
