...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="configs/coffee_martini.yaml")
    ...
    # Keep a global training log under <repo>/output/train.log.
    repo_root = os.path.dirname(os.path.abspath(__file__))
    output_root = os.path.join(repo_root, "output")
    os.makedirs(output_root, exist_ok=True)
    train_log_path = os.path.join(output_root, "train.log")
    train_log_file = open(train_log_path, "a", buffering=1, encoding="utf-8")
    start_stamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    train_log_file.write(f"\n===== [{start_stamp}] New run =====\n")
    train_log_file.write(f"config: {args.config}\n")
    train_log_file.write(f"source_path: {args.source_path}\n")
    train_log_file.write(f"model_path: {args.model_path}\n")
    sys.stdout = _TeeStream(sys.__stdout__, train_log_file)
    sys.stderr = _TeeStream(sys.__stderr__, train_log_file)
